...
class IOUandSek:
    ...
    def evaluate(self):
        ...
        confusion_matrix_seg = self.hist[1:, 1:]
        iou_seg = np.diag(confusion_matrix_seg) / (confusion_matrix_seg.sum(0) + confusion_matrix_seg.sum(1) - np.diag(confusion_matrix_seg))
        miou_seg = np.mean(iou_seg)

        score = 0.5 * miou_cd + 0.5 * miou_seg

        return score, miou_cd, miou_seg
